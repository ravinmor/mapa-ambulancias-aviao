...
function isVehicleInService(operationStatus) {
...
router.get('/api/vehicles/:id/history', asyncHandler(async (req, res) => {
    ...
    const veiculo = await db_1.prisma.veiculo.findUnique({ where: { id }, select: { operationStatus: true } });
    ...
    const points = await db_1.prisma.posicaoOperacao.findMany({
    ...
    const currentPosition = await db_1.prisma.posicaoAtualVeiculo.findUnique({
        where: { veiculoId: id },
        select: { latitude: true, longitude: true, positionAt: true },
    });
...
router.get('/api/vehicles/:id/mission', asyncHandler(async (req, res) => {
    ...
    // acao/posicao nova).
    const operacao = await db_1.prisma.operacao.findFirst({
        where: { veiculoId: id },
    ...
    res.json({
        ...
        currentStatusText: (operacao.currentStatus ? STATUS_OPERACAO_TEXT[operacao.currentStatus] : null) ?? operacao.shortStatus ?? null,
...
router.get('/api/missions/stats', asyncHandler(async (req, res) => {
    ...
    const operacoes = await db_1.prisma.operacao.findMany({
        where: {
    ...
    for (const operacao of operacoes) {
        if (operacao.cancelledAt) {
            if (!inWindow(operacao.assignedAt))
                continue;
        ...
        else if ((0, vehicles_1.isStageDone)(operacao.finishedStatus)) {
            if (!inWindow(operacao.acknowledgedAt))
                continue;
            finished += 1;
        }
    }
