...
exports.runAircraftCycle = runAircraftCycle;
const config_1 = __importDefault(require("./config"));
const db_1 = require("./db");
const opensky_1 = require("./sources/opensky");
...
// sentido leste-oeste.
function distanceToCenter(entry) {
...
async function upsertAircraft(tx, entry, region) {
    const now = new Date();
    const position = {
...
async function runAircraftCycle() {
    const candidates = await (0, opensky_1.fetchAircraft)();
    const candidateByIcao = new Map(candidates.map((c) => [c.icao24, c]));
    const tracked = await db_1.prisma.aircraft.findMany({
    ...
    for (const region of REGIONS) {
        ...
        const free = config_1.default.opensky.slotsPerRegion - occupied;
        if (free <= 0)
            continue;
    ...
    cutoff.setDate(cutoff.getDate() - config_1.default.opensky.historyRetentionDays);
    const deleted = await db_1.prisma.aircraftPositionHistory.deleteMany({ where: { positionAt: { lt: cutoff } } });
    ...
    const staleAircraft = await db_1.prisma.aircraft.deleteMany({
        where: { trackedRegion: null, lastSeenAt: { lt: cutoff } },
    });
    console.log(`[sync-job] aeronaves ok — ${kept.length} mantida(s), ${incoming.length} nova(s), ${releasedIds.length} liberada(s); ` +
        `${deleted.count} ponto(s) expirado(s), ${staleAircraft.count} aeronave(s) antiga(s) removida(s)`);
}
function buildHistoryRow(aircraftId, entry) {
    return {
        aircraftId,
        latitude: entry.latitude,
        longitude: entry.longitude,
