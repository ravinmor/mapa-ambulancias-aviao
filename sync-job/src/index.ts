...
import { Prisma, StatusChamado, StatusOperacao, TipoViagem } from '@prisma/client';
...
async function logSyncError(
  ...
  params: { entityType: string; sourceItemId?: number | null; fieldName?: string; rawValue: string | null; errorType: string; message?: string },
): Promise<void> {
...
const STATUS_CHAMADO_MAP: Record<string, StatusChamado> = {
  'Chamado criado, aguardando aceite do Controle.': StatusChamado.CRIADO_AGUARDANDO_ACEITE,
  'Chamado Cancelado': StatusChamado.CANCELADO,
  'Ida Concluída': StatusChamado.IDA_CONCLUIDA,
  'Volta Concluída': StatusChamado.VOLTA_CONCLUIDA,
};

const STATUS_OPERACAO_MAP: Record<string, StatusOperacao> = {
  'Deslocamento para origem iniciado, aguardando confirmação de chegada na origem.': StatusOperacao.DESLOCANDO_PARA_ORIGEM,
  'Chegada na origem confirmada, aguardando iniciar deslocamento para o destino.': StatusOperacao.CHEGOU_NA_ORIGEM,
  'Deslocamento para o destino iniciado, aguardando confirmação de chegada no destino.': StatusOperacao.DESLOCANDO_PARA_DESTINO,
  'Equipe Resgate concluiu missão.': StatusOperacao.CONCLUIDA_PELO_RESGATE,
  'Equipe do Controle concluiu missão.': StatusOperacao.CONCLUIDA_PELO_CONTROLE,
  'Chamado Cancelado': StatusOperacao.CANCELADA,
  'Operação cancelada pela equipe do Resgate.': StatusOperacao.CANCELADA_PELO_RESGATE,
};

const TIPO_VIAGEM_MAP: Record<string, TipoViagem> = {
  IDA: TipoViagem.IDA,
  Volta: TipoViagem.VOLTA,
  VOLTA: TipoViagem.VOLTA,
  Ida: TipoViagem.IDA,
};

async function mapStatusChamado(tx: Prisma.TransactionClient, raw: string | null, sourceItemId: number): Promise<StatusChamado | null> {
...
async function mapStatusOperacao(tx: Prisma.TransactionClient, raw: string | null, sourceItemId: number): Promise<StatusOperacao | null> {
  if (raw == null) return StatusOperacao.AGUARDANDO_ACEITE; // null na origem = ninguem aceitou ainda, ver nucleo_enums.prisma
...
async function mapTipoViagem(tx: Prisma.TransactionClient, raw: string, sourceItemId: number): Promise<TipoViagem | null> {
...
async function auditFieldChanges(
  ...
  for (const field of criticalFields) {
    ...
    if (before !== null && oldStr === newStr) continue; // sem mudanca — nao audita (nem na 1a insercao, before=null so audita se quiser historico de criacao, decisao: nao audita criacao, so edicao)
    if (before === null) continue; // 1a vez que esse registro aparece — nao e "edicao", e criacao, nao audita
...
async function upsertVeiculo(tx: Prisma.TransactionClient, entry: VeiculoEntry): Promise<void> {
  const data = {
    name: entry.name,
...
async function runVeiculoCycle(): Promise<void> {
...
async function upsertEquipe(tx: Prisma.TransactionClient, entry: EquipeEntry): Promise<void> {
...
async function runEquipeCycle(): Promise<void> {
  if (!config.sharepoint?.equipesUrl && config.dataSource === 'sharepoint') {
  ...
  await prisma.$transaction(async (tx) => {
    for (const entry of entries) await upsertEquipe(tx, entry);
  });
  console.log(`[sync-job] equipes ok — ${entries.length} equipe(s)`);
}

async function upsertColaborador(tx: Prisma.TransactionClient, entry: ColaboradorEntry): Promise<void> {
  const data = {
    name: entry.name,
...
async function runColaboradorCycle(): Promise<void> {
  if (!config.sharepoint?.colaboradoresUrl && config.dataSource === 'sharepoint') {
  ...
  await prisma.$transaction(async (tx) => {
    for (const entry of entries) await upsertColaborador(tx, entry);
  });
  console.log(`[sync-job] colaboradores ok — ${entries.length} colaborador(es)`);
}

async function upsertComposicaoEquipe(tx: Prisma.TransactionClient, entry: ComposicaoEquipeEntry): Promise<boolean> {
...
async function runComposicaoEquipeCycle(): Promise<void> {
  if (!config.sharepoint?.composicaoEquipeUrl && config.dataSource === 'sharepoint') {
  ...
  await prisma.$transaction(async (tx) => {
    for (const entry of entries) {
      if (await upsertComposicaoEquipe(tx, entry)) ok++;
    }
  });
...
async function upsertChamado(tx: Prisma.TransactionClient, entry: ChamadoEntry): Promise<void> {
  const status = await mapStatusChamado(tx, entry.statusRaw, entry.id);

  let motivoCancelamentoId: number | null = null;
  ...
  let tipoChamadoId: number | null = null;
  ...
  await auditFieldChanges(
    tx.chamadoFieldAudit,
    'chamadoId',
    entry.id,
    existing as unknown as Record<string, unknown> | null,
    data as unknown as Record<string, unknown>,
    CHAMADO_CRITICAL_FIELDS,
    null, // "changedBy" nao disponivel na origem pra esses campos — flow nao expoe usuario que editou Regulacao
  );
...
async function runChamadoCycle(): Promise<void> {
  if (!config.sharepoint?.regulationsUrl && config.dataSource === 'sharepoint') {
...
async function upsertOperacao(tx: Prisma.TransactionClient, entry: OperacaoEntry): Promise<boolean> {
  ...
  if (tripType == null) return false; // sem tipo de viagem valido, nao da pra criar (campo obrigatorio) — ja logado em mapTipoViagem
  ...
  const data = {
    ...
    waypointsJson: entry.waypoints as unknown as Prisma.InputJsonValue,
  ...
  await auditFieldChanges(
    tx.operacaoFieldAudit,
    'operacaoId',
    entry.id,
    existing as unknown as Record<string, unknown> | null,
    data as unknown as Record<string, unknown>,
    OPERACAO_CRITICAL_FIELDS,
    entry.assignedByEmail ?? entry.acknowledgedByEmail ?? null,
  );
...
async function runOperacaoCycle(): Promise<void> {
  if (!config.sharepoint?.missionsUrl && config.dataSource === 'sharepoint') {
  ...
  await prisma.$transaction(async (tx) => {
    for (const entry of entries) {
      if (await upsertOperacao(tx, entry)) ok++;
    }
  });
...
async function upsertDisponibilidade(tx: Prisma.TransactionClient, entry: DisponibilidadeEntry): Promise<void> {
  ...
  await auditFieldChanges(
    tx.disponibilidadeFieldAudit,
    'disponibilidadeId',
    entry.id,
    existing as unknown as Record<string, unknown> | null,
    data as unknown as Record<string, unknown>,
    DISPONIBILIDADE_CRITICAL_FIELDS,
    entry.acceptedByUser ?? entry.respondedByUser ?? null,
  );
}

async function runDisponibilidadeCycle(): Promise<void> {
  if (!config.sharepoint?.disponibilidadeUrl && config.dataSource === 'sharepoint') {
...
async function upsertTriagem(tx: Prisma.TransactionClient, entry: TriagemEntry): Promise<boolean> {
  const chamado = await tx.chamado.findUnique({ where: { id: entry.chamadoId } });
  if (!chamado) {
    await logSyncError(tx, {
      entityType: 'Triagem',
...
async function runTriagemCycle(): Promise<void> {
  if (!config.sharepoint?.triagemUrl && config.dataSource === 'sharepoint') {
  ...
  await prisma.$transaction(async (tx) => {
    for (const entry of entries) {
      if (await upsertTriagem(tx, entry)) ok++;
    }
  });
...
async function getDiarioWatermark(): Promise<Date> {
  const latest = await prisma.diarioDaMissao.findFirst({ orderBy: { createdAt: 'desc' }, select: { createdAt: true } });
  return latest?.createdAt ?? new Date(0);
}

async function upsertDiarioEntry(tx: Prisma.TransactionClient, entry: DiarioEntry): Promise<boolean> {
  const chamado = await tx.chamado.findUnique({ where: { id: entry.chamadoId } });
  if (!chamado) {
    await logSyncError(tx, {
  ...
  let disponibilidadeId: number | null = null;
...
async function runDiarioCycle(): Promise<void> {
  if (!config.sharepoint?.diarioUrl && config.dataSource === 'sharepoint') {
  ...
  await prisma.$transaction(async (tx) => {
    for (const entry of entries) {
      if (await upsertDiarioEntry(tx, entry)) ok++;
    }
  });
...
async function getVeiculoPosicaoWatermark(veiculoId: number): Promise<number> {
  ...
  const latest = await prisma.posicaoOperacao.findFirst({
    where: { veiculoId, id: { lt: BACKFILL_ID_OFFSET } },
    orderBy: { id: 'desc' },
    select: { id: true },
  });
  return latest?.id ?? 0;
...
const OPERACAO_ATIVA_STATUS: StatusOperacao[] = [
  StatusOperacao.DESLOCANDO_PARA_ORIGEM,
  StatusOperacao.CHEGOU_NA_ORIGEM,
  StatusOperacao.DESLOCANDO_PARA_DESTINO,
];

async function runPosicaoCycle(): Promise<void> {
  if (!config.sharepoint?.trackingUrl && config.dataSource === 'sharepoint') {
    console.log('[sync-job] posicao pulado — POWER_AUTOMATE_TRACKING_URL nao configurado ainda');
    return;
  }

  const operacoesAtivas = await prisma.operacao.findMany({
  ...
  for (const veiculoId of veiculoIds) {
    try {
      ...
      await prisma.$transaction(async (tx) => {
        for (const entry of entries) {
          if (entry.operacaoId == null) continue; // sem operacao, nao ha onde ligar a posicao no nucleo
          const operacao = await tx.operacao.findUnique({ where: { id: entry.operacaoId } });
          if (!operacao) {
            await logSyncError(tx, {
  ...
  await prisma.posicaoOperacao.deleteMany({ where: { positionAt: { lt: cutoff } } });
}

async function runPosicaoBackfillCycle(): Promise<void> {
  if (!config.sharepoint?.historyBackfillUrl && config.dataSource === 'sharepoint') {
  ...
  if (operacoesAtivas.length === 0) return;
...
console.log(
  `[sync-job] iniciando (nucleo resgate) — fonte: ${config.dataSource}, veiculos: ${config.syncIntervalMs}ms, posicao: ${config.historySyncIntervalMs}ms, backfill: ${config.historyBackfillIntervalMs}ms, diario: ${config.missionEventSyncIntervalMs}ms, operacoes: ${config.missionSyncIntervalMs}ms, chamados: ${config.regulationSyncIntervalMs}ms, disponibilidade: ${config.disponibilidadeSyncIntervalMs}ms, triagem: ${config.triagemSyncIntervalMs}ms`,
);
startLoop('veiculos', config.syncIntervalMs, runVeiculoCycle);
...
startLoop('equipes', config.equipeSyncIntervalMs, runEquipeCycle);
startLoop('colaboradores', config.equipeSyncIntervalMs, runColaboradorCycle);
startLoop('composicao de equipe', config.equipeSyncIntervalMs, runComposicaoEquipeCycle);
startLoop('posicao', config.historySyncIntervalMs, runPosicaoCycle);
startLoop('backfill de posicao', config.historyBackfillIntervalMs, runPosicaoBackfillCycle);
startLoop('diario da missao', config.missionEventSyncIntervalMs, runDiarioCycle);
...
startLoop('chamados', config.regulationSyncIntervalMs, runChamadoCycle);
startLoop('operacoes', config.missionSyncIntervalMs, runOperacaoCycle);
startLoop('disponibilidade', config.disponibilidadeSyncIntervalMs, runDisponibilidadeCycle);
startLoop('triagem', config.triagemSyncIntervalMs, runTriagemCycle);
