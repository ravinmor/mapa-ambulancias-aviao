...
import { DataSource, FleetEntry, HistoryEntry, MissionEventEntry } from '../types';
...
export const sharepointSource: DataSource = {
  ...
  async fetchHistorySince(since: Date): Promise<HistoryEntry[]> {
    if (!config.sharepoint?.trackingUrl) {
      throw new Error('POWER_AUTOMATE_TRACKING_URL ausente');
    }
    const items = await callFlow(config.sharepoint.trackingUrl, { desde: since.toISOString() });
