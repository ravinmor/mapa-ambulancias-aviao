...
function mapPosicaoItems(
  items: ListItemFields[],
  field: typeof POSICAO_FIELD | typeof POSICAO_BACKFILL_FIELD,
): PosicaoEntry[] {
  const entries: PosicaoEntry[] = [];
...
export const sharepointSource: DataSource = {
  async fetchVeiculos(): Promise<VeiculoEntry[]> {
    if (!config.sharepoint) throw new Error('Configuracao do SharePoint ausente');
  ...
  async fetchPosicaoForVeiculo(veiculoId: number, sinceItemId: number): Promise<PosicaoEntry[]> {
    if (!config.sharepoint?.trackingUrl) throw new Error('POWER_AUTOMATE_TRACKING_URL ausente');
  ...
  async fetchPosicaoBackfillForOperacao(operacaoId: number): Promise<PosicaoEntry[]> {
    if (!config.sharepoint?.historyBackfillUrl) throw new Error('POWER_AUTOMATE_HISTORY_BACKFILL_URL ausente');
    const items = await callFlow(config.sharepoint.historyBackfillUrl, { operacao: String(operacaoId) });
    ...
    return mapPosicaoItems(items, POSICAO_BACKFILL_FIELD).map((entry) => ({ ...entry, id: entry.id + BACKFILL_ID_OFFSET }));
  },

  async fetchRecentChamados(): Promise<ChamadoEntry[]> {
    if (!config.sharepoint?.regulationsUrl) throw new Error('POWER_AUTOMATE_REGULATIONS_URL ausente');
    const items = await callFlow(config.sharepoint.regulationsUrl);
    const entries: ChamadoEntry[] = [];
    for (const item of items) {
      const id = Number(item.ID ?? item.Id);
  ...
  async fetchRecentOperacoes(): Promise<OperacaoEntry[]> {
    if (!config.sharepoint?.missionsUrl) throw new Error('POWER_AUTOMATE_MISSIONS_URL ausente');
    const items = await callFlow(config.sharepoint.missionsUrl);
    const entries: OperacaoEntry[] = [];
    for (const item of items) {
      const id = Number(item.ID ?? item.Id);
  ...
  async fetchEquipes(): Promise<EquipeEntry[]> {
    if (!config.sharepoint?.equipesUrl) throw new Error('POWER_AUTOMATE_EQUIPES_URL ausente');
    const items = await callFlow(config.sharepoint.equipesUrl);
    return items
  ...
  async fetchColaboradores(): Promise<ColaboradorEntry[]> {
    if (!config.sharepoint?.colaboradoresUrl) throw new Error('POWER_AUTOMATE_COLABORADORES_URL ausente');
    const items = await callFlow(config.sharepoint.colaboradoresUrl);
    return items
  ...
  async fetchComposicaoEquipe(): Promise<ComposicaoEquipeEntry[]> {
    if (!config.sharepoint?.composicaoEquipeUrl) throw new Error('POWER_AUTOMATE_COMPOSICAO_EQUIPE_URL ausente');
    const items = await callFlow(config.sharepoint.composicaoEquipeUrl);
    const entries: ComposicaoEquipeEntry[] = [];
    for (const item of items) {
      const id = Number(item.ID ?? item.Id);
      ...
      if (!Number.isInteger(id) || equipeId == null || colaboradorId == null) continue;
      entries.push({
        id,
        equipeId,
        colaboradorId,
  ...
  async fetchRecentDisponibilidades(): Promise<DisponibilidadeEntry[]> {
    if (!config.sharepoint?.disponibilidadeUrl) throw new Error('POWER_AUTOMATE_DISPONIBILIDADE_URL ausente');
    const items = await callFlow(config.sharepoint.disponibilidadeUrl);
    const entries: DisponibilidadeEntry[] = [];
    for (const item of items) {
      const id = Number(item.ID ?? item.Id);
  ...
  async fetchRecentTriagens(): Promise<TriagemEntry[]> {
    if (!config.sharepoint?.triagemUrl) throw new Error('POWER_AUTOMATE_TRIAGEM_URL ausente');
    const items = await callFlow(config.sharepoint.triagemUrl);
    const entries: TriagemEntry[] = [];
    for (const item of items) {
      const id = Number(item.ID ?? item.Id);
      const chamadoId = toNumber(item[TRIAGEM_FIELD.chamadoId]);
      if (!Number.isInteger(id) || chamadoId == null) continue;
      entries.push({
        id,
        chamadoId,
  ...
  async fetchDiarioSince(since: Date): Promise<DiarioEntry[]> {
    if (!config.sharepoint?.diarioUrl) throw new Error('POWER_AUTOMATE_DIARIO_URL ausente');
    const items = await callFlow(config.sharepoint.diarioUrl, { desde: since.toISOString() });
