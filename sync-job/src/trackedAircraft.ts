import type { TrackedAircraft as TrackedAircraftRow } from '@prisma/client';
...
import { fetchAdsbdbAircraft, fetchAdsbdbRoute } from './sources/adsbdbSource';
import { fetchRecentFlights } from './sources/openskyFlightsSource';
...
// no estagio.
function deriveStage(previous: Stage | null, state: TrackedAircraftState, distanceToDestinationKm: number | null): Stage {
  ...
  let candidate: Stage;
  if (distanceToDestinationKm != null && distanceToDestinationKm <= APPROACH_PROXIMITY_KM) candidate = 'APROXIMACAO';
...
function staleLandedThresholdMs(distanceKm: number): number {
  if (distanceKm <= APPROACH_CLOSE_PROXIMITY_KM) return APPROACH_CLOSE_RECHECK_INTERVAL_MS * 3; // ~6s
  if (distanceKm <= APPROACH_MID_PROXIMITY_KM) return APPROACH_MID_RECHECK_INTERVAL_MS * 2; // 60s
...
function approachDistanceKm(existing: TrackedAircraftRow | null): number | null {
  if (existing?.stage !== 'APROXIMACAO') return null;
  if (existing.latitude == null || existing.longitude == null) return null;
  if (existing.destinationLatitude == null || existing.destinationLongitude == null) return null;
  return distanceKm(existing.latitude, existing.longitude, existing.destinationLatitude, existing.destinationLongitude);
}

function isNearDestinationInApproach(existing: TrackedAircraftRow | null): boolean {
...
interface FlightTimingExisting {
  flightStartedAt: Date | null;
  flightEndedAt: Date | null;
  pendingDepartureAt: Date | null;
  pendingArrivalAt: Date | null;
}

interface FlightTimingUpdate {
  flightStartedAt?: Date | null;
  flightEndedAt?: Date | null;
  pendingDepartureAt?: Date | null;
  pendingArrivalAt?: Date | null;
...
function resolveFlightTiming(existing: FlightTimingExisting | null, state: TrackedAircraftState): FlightTimingUpdate {
  const looksAirborne =
    !state.onGround && ((state.altitude ?? 0) > AIRBORNE_MIN_ALTITUDE_M || (state.velocity ?? 0) > AIRBORNE_MIN_VELOCITY_MS);
...
async function syncAircraftMetadata(icao24: string, existing: TrackedAircraftRow | null): Promise<void> {
  if (existing?.registration) return;
  try {
    const metadata = await fetchAdsbdbAircraft(icao24);
    if (metadata) {
      await prisma.trackedAircraft.update({ where: { icao24 }, data: metadata });
...
async function syncFlightHistory(icao24: string, trackedAircraftId: number, existing: TrackedAircraftRow | null, now: Date): Promise<void> {
  const historyElapsedMs = existing?.historySyncedAt ? now.getTime() - existing.historySyncedAt.getTime() : Number.POSITIVE_INFINITY;
  if (historyElapsedMs < HISTORY_SYNC_INTERVAL_MS) return;
  ...
  try {
    ...
    const legs = await fetchRecentFlights(icao24, sinceSeconds);
    if (legs.length > 0) {
      await prisma.trackedAircraftFlightHistory.createMany({
        data: legs.map((leg) => ({
          trackedAircraftId,
    ...
    await prisma.trackedAircraft.update({ where: { icao24 }, data: { historySyncedAt: now } });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
...
async function checkOne(icao24: string): Promise<void> {
  ...
  if (!state) {
    ...
    const flightTimingOnOffline: { flightEndedAt?: Date; pendingArrivalAt?: null; stage?: Stage; onGround?: boolean } = {};
    if (existing?.pendingArrivalAt) {
    ...
    if (flightTimingOnOffline.flightEndedAt) {
      console.log(
        `[sync-job] aeronave monitorada (${icao24}) POUSOU (confirmado por silencio${existing?.pendingArrivalAt ? ' pos-pouso' : ' perto do destino em aproximacao'}) — ${flightTimingOnOffline.flightEndedAt.toISOString()}`,
      );
    }
  ...
  const staleForTooLong =
    existingPositionAt != null && approachDistance != null && now.getTime() - existingPositionAt.getTime() > staleLandedThresholdMs(approachDistance);
  if (existingPositionAt != null && isSameOldPosition && staleForTooLong && isConfirmedAirborneNow && isNearDestinationInApproach(existing)) {
    ...
    console.log(
      `[sync-job] aeronave monitorada (${icao24}) POUSOU (confirmado por estagnacao — OpenSky repetindo o mesmo fix perto do destino em aproximacao) — ${existingPositionAt.toISOString()}`,
    );
    await syncAircraftMetadata(icao24, existing);
  ...
  if (state.callsign && state.callsign !== existing?.routeCallsign) {
    try {
      const route = await fetchAdsbdbRoute(state.callsign);
  ...
  // atrasado).
  const freshDistanceToDestinationKm =
